...
def balanced(m):
    ...
    left_thing, right_thing = end(left_arm), end(right_arm)    
    overall_balance = total_weight(left_thing) * length(left_arm) == total_weight(right_thing) * length(right_arm)
    return balanced(left_thing) and balanced(right_thing) and overall_balance 
...
def has_path(t, word):
    ...
    # 递归调用过程：word中最前的字符是匹配的，并且之后也有路径能够匹配到剩余的字符
    if label(t) == word[0]:
        for b in branches(t):
...
def quadratic(x, a, b, c):
    ...
    if x1 <= s_axis <= x2:
        if a > 0:
            return interval(value, max(y1, y2))
        else:
            return interval(min(y1, y2), value)
    else:
